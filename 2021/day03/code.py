...
for bit_index in range(len(data[0])):
    ...
    most_common_bit = int(
        sum([x[bit_index] for x in oxygen_generator_rating])
        >= len(oxygen_generator_rating) / 2
    )
    least_common_bit = int(
        sum([x[bit_index] for x in co2_scrubber_rating]) < len(co2_scrubber_rating) / 2
    )

    if len(oxygen_generator_rating) > 1:
        oxygen_generator_rating = list(
            filter(lambda x: x[bit_index] == most_common_bit, oxygen_generator_rating)
        )
    if len(co2_scrubber_rating) > 1:
        co2_scrubber_rating = list(
            filter(lambda x: x[bit_index] == least_common_bit, co2_scrubber_rating)
        )
